...
outsheet using table10A.xls if sfr, replace
outsheet using table10A_mfr.xls if !sfr, replace
...
replace landval = landval / 1e9
replace shape_area = shape_area * 10.7639 / 1e9
outsheet using table10A_nonresidential.xls, replace
*/
...
collapse (sum) landval shape_area (mean) nparcel, by(county)
format landval shape_area %14.0f 
outsheet using july19_office_landval.csv, comma replace
