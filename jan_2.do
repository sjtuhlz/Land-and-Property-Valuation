//Use census tract level data
//Jan 2, 2012
//  Vrm = Vrs / Rrs * Rrm
// Single and Multi are calculated together~
//comparing results using Yizhen's updated floor area data vs originally available floor area data

capture log close
log using jan_2, replace
clear
clear matrix
set mem 1g
set more off
...
drop if missing(x) 
save residential_six_final_jan2.dta, replace
*/
...
outsheet  vsq_mz1 vsq_mz2 rsq_mz1 rsq_mz2 mz name using mz_single_jan2.csv, comma replace
...
save jan2_five_residential.dta, replace

drop if missing(vsq_mz1) | missing(vsq_mz2)
duplicates drop vsq_mz1 vsq_mz2, force
outsheet  vsq_mz1 vsq_mz2 rsq_mz1 rsq_mz2 mz name using mz_multi_jan2.csv, comma replace
