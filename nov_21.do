...
rename id tractid
rename near_ro_di fwy   //note that the fwy and ocean are calculated by Ross Guo, and they are in miles already.
...
use parcel_or_residential_merged.dta, clear
...
foreach var of varlist hos hrs hrm rrs rrm vos vom {
	drop `var'
...
save nov21_residential.dta, replace



duplicates drop vsq_mz_single vsq_mz_multi, force
//esttab * using Oct10, b p obslast label mtitles rtf replace
outsheet  vsq_mz_single rsq_mz_single vsq_mz_multi rsq_mz_multi mz name using mz_residential_or_nov21.csv, comma replace
