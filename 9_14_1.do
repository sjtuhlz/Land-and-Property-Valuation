...
xi: reg lgvsqft fsub cbd freeway ocean age i.saleyr if insample
...
foreach var of varlist `land_use' {
	...
	xi: reg age fsub cbd freeway ocean i.city if `var' == 1 
...
foreach supervar of varlist `land_use' {
	...
	xi: reg  lgvsq  fsub cbd freeway ocean  i.city i.saleyr if sample
...
/*To impute saleprice from totval07 for SB & IM*/
	...
	scalar xr = scalar(tsp) / scalar(tv)
/*XR part end*/
...
/*imputation of rent*/
	...
	outsheet  scagxyid vsq_2000 rsq_2000 floor_area mz using 9_16_`supervar'.csv if sample, comma
