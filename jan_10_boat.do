//jan 11, 2012
//a modified version of jan_10.do
//boats are parts of mobile home

//=========== CENSUS data with TractID, H*, Rr*, Vo*  (boat was included in mobile home major category) 
cd "D:\summer, 2011\Third Project\DT_download"
local counties la VT OR SB RV
foreach lc of local counties {
    insheet using 3rd_DTDownload_`lc'.csv, clear
    ...
    save 3rd_DTDownload_`lc'.dta, replace
}
use 3rd_DTDownload_la.dta, clear
local counties VT OR SB RV
foreach lc of local counties {
    append using 3rd_DTDownload_`lc'.dta, force
}
...
save 3rd_DTDownload.dta, replace
...
drop _merge*
rename id tractid
merge m:1 tractid using "D:\summer, 2011\Third Project\DT_download\3rd_DTDownload.dta"
save census_mz_3category_boat.dta, replace
//note that Wenwen's file of censusid_mz correspondence includes Imperial County, while my 2nd_DTDownload.dta doesnot



//collapse Wenwen's census file into mz level
use census_mz_3category_boat.dta, clear
...
use residential_six_final.dta, clear
capture drop _merge
merge m:1 mz using "D:\summer, 2011\Third Project\model_zone_level_census\census_mz_3category_boat.dta", force
save residential_six_final_3category_boat.dta, replace  //3categories suffix means here mobile homes were treated as a equally improtant major category as Single and Multi



use residential_six_final_3category_boat.dta, clear
//data processing
...
forvalues i = 1/3 {
    outsheet mz name vsq_mz* rsq_mz*`i'  vos vom vomb vrs`i' vrm`i' vrmb`i' rrs rrm rrmb vs ros rom romb using jan_10_`i'.csv, comma replace
}
