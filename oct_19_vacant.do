//Based on Oct 14, 2012


*** Goals Oct 19, 2012
* (1) Run a regression without differentiating developable and undevelopable, to justify Ross' work.
* (2) Compare the sets of regression coefficients on accessibility measures. 
* (3) Compare estimates aggregate vacant land values using different schemes.
...
log using oct_19_vacant, replace
set more off
set mem 800m
...
*********************
//No differentiation of developability  
*********************

xtreg  lgvsq  fsub cbd fwy ocean i.saleyr i.lu_08 i.scag_gp_co if !missing(saleyr), fe
estimates store together

estimates save together, replace
...
forvalues j = 0/2 {
    gen tsample_`j' = !missing(v2000_`j')
        gen tsample = tsample_`j' 
        bysort mz: egen area_total = total(shape_area) if tsample
        by mz: egen val_total = total(v2000_`j') if tsample
        gen vsq_mz_`j' = val_total / area_total  
        drop area_total val_total
	drop tsample  
}

save oct_19_vacant_results.dta, replace


collapse vsq_mz* (sum) shape_area* , by(mz)
outsheet  mz vsq_mz*  shape_area* using oct_19_vacant.csv, comma replace
*/
...
esttab together developable undevelopable using oct_19_est_coef.rtf, replace keep(fsub cbd fwy ocean) mtitles(combined developable undevelopable)


esttab together developable undevelopable using oct_19_est_coef_complete.rtf, mtitles(combined developable undevelopable) replace  
