...
xi: reg floor_area shape_area i.lu_08 i.city
...
replace insample = (county == "OR" | county == "RV") & `tp'  
...
if "`tp'" == "multi" | "`tp'" == "mbhome" {
    reg lgvsq  fsub cbd cbd2 fwy ocean ocean2 ib(freq).city_n i.saleyr  ib(freq).lu_08 if insample & !missing(saleyr)
    ...
    //regression and imputation using TOTVALUE07
    reg lgvsq_tot fsub cbd cbd2 fwy ocean ocean2 ib(freq).city_n i.saleyr ib(freq).lu_08 if insample & !missing(saleyr)
    levelsof saleyr if e(sample), local(lvsaleyr)
    foreach lv of local lvsaleyr {
...
//====================== LA, VT ==========================
replace insample = (county == "LA" | county == "VT")  & `tp' 
...
*    replace insample = county == "SB" & `tp'
*    replace v2000 = exp(lnv2000)  * scalar(xr_`tp') if insample 
}
if "`tp'" == "multi" | "`tp'" == "mbhome" {
    reg lgvsq_tot fsub cbd cbd2 fwy ocean ocean2 ib(freq).city_n i.saleyr i.lu_08 if insample & !missing(saleyr)
    levelsof saleyr if e(sample), local(lvsaleyr)
    foreach lv of local lvsaleyr {
            capture replace  lgvsq2000_tot =  lgvsq_tot - _b[`lv'.saleyr] + _b[2000.saleyr] if saleyr == `lv'  & e(sample)
        }
...
if "`tp'" == "multi" | "`tp'" == "mbhome" {
    ...
    levelsof saleyr if e(sample), local(lvsaleyr)
    foreach lv of local lvsaleyr {
            replace  lgvsq2000_tot =  lgvsq_tot - _b[`lv'.saleyr] + _b[2000.saleyr] if saleyr == `lv'  & e(sample)
        }
    replace saleyr = 2000 if e(sample)==0  & insample
    sum saleyr if e(sample) == 0
    predict lgvsq_p if insample
    replace lgvsq2000_tot = lgvsq_p if e(sample) == 0 & insample
    drop  lgvsq_p
...
save results_jan10_unix_july27_update.dta, replace
...
collapse (sum) shape_area floor_area v2000 id_floorarea id_shape_area, by(county sfr)
